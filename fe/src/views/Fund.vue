...
// 使用 mock 数据模拟搜索
import { mockFunds } from "@/lib/api.js";
const handleSearch = () => {
  searchQuery.value = searchInput.value.trim();
  if (!searchQuery.value) return;
  ...
  setTimeout(() => {
    // 模拟根据 searchQuery 过滤
    const result = mockFunds.filter((stock) =>
...
import { fundPrices } from "@/lib/api.js";
const fetchEODData = (stock, onlyPrice = false) => {
  const mock = fundPrices.find((s) => s.ticker === stock.ticker);
  if (!mock) return;
    ...
    <div class="mx-auto max-w-6xl space-y-10 p-6">
      ...
      <div class="sticky top-16 z-10 flex gap-3 rounded-lg bg-white p-4 shadow">
        <input
          v-model="searchInput"
          type="text"
          placeholder="Enter fund name or ticker"
      ...
      <Card>
        <CardContent>
          <h2 class="mb-4 text-xl font-bold text-gray-800">Available Funds</h2>
          <Table class="w-full">
            <TableHeader>
              <TableRow class="bg-gray-100">
